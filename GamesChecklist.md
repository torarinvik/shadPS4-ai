...
| [x] | UFC 3 | CUSA06534 | Renders — the EA Sports UFC 3 title/splash screen displays correctly (confirmed visually), game stays alive past ~5:00 playtime. Then crashes with a MoltenVK device loss: `[mvk-error] VK_ERROR_OUT_OF_DEVICE_MEMORY: Lost VkDevice after MTLCommandBuffer execution failed (code 9): Invalid Resource (kIOGPUCommandBufferCallbackErrorInvalidResource)`, after which `vk_presenter.cpp GetRenderFrame` asserts on `ErrorDeviceLost` (repeating). This is the SAME device-loss bug as Madden NFL 24, hit during the videoout compute-storage path (1920x1080 R8G8B8A8 surfaces, `pgm=0xe5b29094`/`0xf119d9d9`, `is_videoout_storage=true`). Two theories tested and ruled out: (1) freed-image-while-in-flight — image destruction already defers past the GPU tick; (2) illegal sRGB storage image — running with `SHADPS4_VIDEOOUT_UNORM=1` (forces the surface R8G8B8A8Srgb -> Unorm) did NOT prevent the device loss. ROOT CAUSE FOUND via Metal API validation (`MVK_CONFIG_DEBUG=1` + `METAL_DEVICE_WRAPPER_TYPE=1` + sync submits): Metal reports "The following Metal object is being destroyed while still required to be alive by the command buffer ... CommandPool: Command Buffer 2" naming two 16 KB private `AGXG17GFamilyBuffer`s (guest buffers `0x284628000:0x4000` and `0x28462c000:0x4000`). So it is NOT the videoout image — it is a small guest BUFFER freed while a recorded-but-not-yet-completed Metal command buffer still references it. These are buffer-cache buffers torn down on the overlap/merge path (`BufferCache::ExpandBuffer` -> `DeleteBuffer(overlap_id)`, buffer_cache.cpp:653) and/or guest unmap (`Rasterizer::UnmapMemory` -> `buffer_cache.InvalidateMemory`). `DeleteBuffer` defers `slot_buffers.erase` via `scheduler.DeferOperation`, which captures `gpu_tick = MasterSemaphore::CurrentTick()`. The deferred destroy becomes eligible when `KnownGpuTick() >= gpu_tick`, but `KnownGpuTick` only refreshes at submit and `current_tick` does not advance until the in-progress command buffer is actually submitted — so a `PopPendingOperations` triggered by an unrelated wait can run the destroy (vmaDestroyBuffer) before the command buffer holding the copy is submitted/completed, leaving Metal with a dangling buffer. This is a scheduler tick-accounting / resource-lifetime bug, shared with Madden NFL 24. Partial fix applied (`scheduler.DeferOperationAfterSubmit`, defers to `CurrentTick()+1`): three buffer-cache paths that record a buffer into the current command buffer and then free it via `DeferOperation` at `CurrentTick()` were corrected — `DeleteBuffer` (overlap/merge + GC), the large-upload temp buffer in `UploadCopies`, and the large-transfer temp buffer in `WriteDataBuffer`. This pushed the device loss from ~15s to ~49s and, confirmed via Metal validation naming, eliminated the originally-failing guest buffers (16 KB `0x284628000`, 245 KB `0x26bfa8000`). Regression-checked: New Super Lucky's Tale still runs clean (no device-loss) and Crash Bandicoot's unrelated LegalScreen crash reproduces identically with and without the fix, so the change is safe. It is NOT a complete fix — UFC 3 still device-loses. Investigation continued: the next named resource was the unlabeled 128 KB scratch buffer from `TileManager::GetScratchBuffer` (the detile/tile compute path, also recorded-then-`DeferOperation`-freed). Applying the same `DeferOperationAfterSubmit` deferral there moved the failure again (to "Command Buffer 5") but did NOT stop the device loss, and the wallclock-to-crash timings across attempts (~15s -> ~49s -> ~41s) are within noise, so that extra change was not demonstrably helpful and was reverted (only the buffer-cache fix is committed). Conclusion: the `+1 tick` per-site deferral is the right direction for the specific named guest buffers (those no longer fail) but is NOT the complete fix — the resource keeps moving across many recorded-then-freed sites, which suggests a more fundamental command-buffer/resource-residency lifetime issue (possibly multiple in-flight Metal command buffers vs the single-tick model, or descriptor/argument-buffer residency) rather than a finite set of leaky call sites. Needs a structural fix (e.g. tracking last-used tick per resource, or keeping freed GPU resources alive until all in-flight command buffers that referenced them complete) rather than more per-site patches. Earlier flip-count "black screen" readings were a false negative (it presents via the flip-event path, not sceVideoOutSubmitFlip). Current bucket: renders title screen; specific guest-buffer use-after-frees fixed and verified, but a deeper resource-lifetime device-loss remains (shared with Madden NFL 24). | Not tested | Not tested |
