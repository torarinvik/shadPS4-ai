...
| [x] | UFC 3 | CUSA06534 | Retested after image-view range and depth-stencil compatibility hardening. The old `GetBarriers` out-of-bounds layer assert is fixed, the Metal/MoltenVK `R16Uint` view-of-depth abort is fixed, and incompatible direct image copies are skipped instead of crashing. Short watchdog run no longer crashed and reached playtime updates around 3 minutes before the external timeout killed it. Needs manual visual/audio assessment. | Not tested | Not tested |
...
| [x] | Madden NFL 24 | CUSA37089 | Tested. Gets relatively far: loads 186 modules, compiles 76 shaders, and opens VideoOut with `sceVideoOutOpen` (x4) and `RegisterBuffers` (x4). One non-fatal `MapMemory: Unable to map 0x4000 bytes at address 0xfbffffc000` is logged but the game keeps running past it (no crash). It then settles into a repeating DirtySDK `sceNetEpollCreate` (`DirtySDK_SocketRecvThread`) network loop and keeps advancing playtime (1:00+), but never submits a flip (`sceVideoOutSubmitFlip` = 0), so nothing is presented. Run ended by manual quit, not a crash. User observed a black screen with no behavior. Current bucket: silent black screen — network/online-init stall with no presented frame, rather than the fatal `0x4000000000`-family fixed-mapping crash. | Not tested | Not tested |
...
| Madden NFL 24 | CUSA37089 | Opens VideoOut and registers buffers, but never flips; stuck in DirtySDK network init loop with a black screen. |
