...
| [x] | Hasbro Family Fun Pack | CUSA03312 | Retested. Launcher starts and selecting Monopoly with X successfully restarts into `monopoly/monopoly.self`. The previous Monopoly crash immediately after selection was tied to `sceUserServiceGetUserName` receiving the system user id `0xff`; user-service name/color lookups now fall back to the default local user for system ids, and the old `Read from address 0x20` crash was not reproduced. Direct `monopoly.self` launch works, and the launcher handoff now uses POSIX in-place `execvp` on macOS/Linux instead of fork + child wait, avoiding the odd parent/child quit/crash-report shape. Current remaining note: Monopoly keeps querying invalid/empty user slots (`user_id = -1`, occasionally `8`) for name/color, producing noisy user-service errors but continuing to run. |
...
| [-] | Tokyo Twilight Ghost Hunters Daybreak: Special Gigs | CUSA06045 | Tested. Boots and reaches in-game visual-novel scenes. Input combo prompt `R1 + Square` maps to keyboard `U + Z`; game appeared to work normally during the initial test. |
...
| [-] | Katamari Damacy Reroll | CUSA24361 | Tested. Appears to work/playable during manual play. Strict black-screen watchdog stayed nonblack and no GPU wait timeout or crash was observed. Log is noisy with repeated `Unexpected metadata read by a shader (texture)` warnings, but they do not currently block gameplay. |
