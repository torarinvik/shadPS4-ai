...
struct PM4CmdReleaseMem {
    ...
    void SignalFence(auto&& write_mem, auto&& signal_irq, auto&& gds_to_mem) const {
        switch (data_sel.Value()) {
        case DataSelect::Data32Low: {
            write_mem(Address<VAddr>(), DataDWord(), sizeof(u32));
            break;
        }
        case DataSelect::Data64: {
            write_mem(Address<VAddr>(), DataQWord(), sizeof(u64));
            break;
        }
        case DataSelect::GpuClock64: {
            write_mem(Address<VAddr>(), GetGpuClock64(), sizeof(u64));
            break;
        }
        case DataSelect::PerfCounter: {
            write_mem(Address<VAddr>(), GetGpuPerfCounter(), sizeof(u64));
            break;
        }
